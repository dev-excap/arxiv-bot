...
def render_paper(paper_entry: Paper, counter: int) -> str:
    MAX_CHAR_LIMIT = 3000  # Slack's character limit for text in a block

    # Construct the paper_string as before
    ...
    paper_string = (
        f"<{arxiv_url}|*{counter}. {title.replace('&', '&amp;')}*>\n"
    ...
    if "RELEVANCE" in paper_entry and "NOVELTY" in paper_entry:
        ...
        paper_string += f"*Relevance*: {relevance}\t*Novelty*: {novelty}\n"
    ...
    if len(paper_string) > MAX_CHAR_LIMIT:
        paper_string = paper_string[:MAX_CHAR_LIMIT - 3] + "..."
